...
export function registerWhyCommand(
  ...
  program
    ...
    .action(async (target: string) => {
      ...
      // For each unique commit, try to find a Lore atom
      const atoms: LoreAtom[] = [];
      const seenLoreIds = new Set<string>();

      for (const hash of commitHashes) {
        // Get the raw commit to extract trailers
        const rawCommits = await gitClient.log(['-1', hash]);
        if (rawCommits.length === 0) {
          continue;
        }

        // Try to find a Lore atom for this commit
        // We search the full log for commits matching this hash
        const allAtoms = await atomRepository.findAll({ limit: 1000 });
        const matchingAtom = allAtoms.find((a) => a.commitHash === hash);

        if (matchingAtom && !seenLoreIds.has(matchingAtom.loreId)) {
          atoms.push(matchingAtom);
          seenLoreIds.add(matchingAtom.loreId);
        }
      }
