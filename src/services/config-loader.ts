  ...
  private toPartialConfig(parsed: Record<string, unknown>): Partial<MutableLoreConfig> {
    ...
    if (parsed.validation && typeof parsed.validation === 'object') {
      ...
      config.validation = {
        ...
        maxMessageLines: typeof validation.maxMessageLines === 'number'
          ? validation.maxMessageLines
          : DEFAULT_CONFIG.validation.maxMessageLines,
        intentMaxLength: typeof validation.intentMaxLength === 'number'
    ...
    if (parsed.stale && typeof parsed.stale === 'object') {
      ...
      config.stale = {
        olderThan: typeof stale.olderThan === 'string' ? stale.olderThan : DEFAULT_CONFIG.stale.olderThan,
        driftThreshold: typeof stale.driftThreshold === 'number'
    ...
    if (parsed.output && typeof parsed.output === 'object') {
      ...
      const format = output.defaultFormat;
      config.output = {
    ...
    if (parsed.follow && typeof parsed.follow === 'object') {
      ...
      config.follow = {
        maxDepth: typeof follow.maxDepth === 'number' ? follow.maxDepth : DEFAULT_CONFIG.follow.maxDepth,
      };
    }
