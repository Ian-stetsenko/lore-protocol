...
export class StalenessDetector {
  ...
  private checkAge(
    ...
    if (ageMs > maxAgeMs) {
      ...
      reasons.push(`Age: atom is ${ageDescription} old (threshold: ${this.config.stale.olderThan})`);
    }
  }
    ...
    for (const filePath of atom.filesChanged) {
      try {
        ...
        if (commitsSince > this.config.stale.driftThreshold) {
          reasons.push(
            `Drift: ${filePath} has ${commitsSince} commits since this atom (threshold: ${this.config.stale.driftThreshold})`,
          );
        }
      } catch {
   ...
   */
  private checkLowConfidence(atom: LoreAtom, reasons: string[]): void {
    if (atom.trailers.Confidence === 'low') {
      reasons.push('Low confidence: atom is marked as Confidence: low');
    }
  }
    ...
    for (const directive of atom.trailers.Directive) {
      ...
      while ((match = untilPattern.exec(directive)) !== null) {
        ...
        if (expiryDate !== null && now > expiryDate) {
          reasons.push(
            `Expired hint: directive "${directive}" has expired [until:${dateStr}]`,
          );
        }
      }
    }
  ...
  private checkOrphanedDependency(
    ...
    for (const depId of atom.trailers['Depends-on']) {
      ...
      if (depStatus && depStatus.superseded) {
        reasons.push(
          `Orphaned dependency: depends on ${depId} which is superseded by ${depStatus.supersededBy}`,
        );
      }
    }
  }
