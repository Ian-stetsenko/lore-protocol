...
describe('JsonFormatter', () => {
  ...
  describe('formatQueryResult', () => {
    ...
    it('should use snake_case field names', () => {
      ...
      expect(parsed.atoms[0].lore_id).toBe('a1b2c3d4');
      expect(parsed.atoms[0].commit_hash).toBe('abc1234567890');
      expect(parsed.atoms[0].files_changed).toEqual(['src/auth.ts']);
    });
    ...
    it('should format dates as ISO strings', () => {
      ...
      expect(parsed.atoms[0].date).toBe('2025-06-15T14:30:00.000Z');
    ...
    it('should include supersession status', () => {
      ...
      expect(parsed.atoms[0].superseded).toBe(true);
      expect(parsed.atoms[0].superseded_by).toBe('e5f6a7b8');
    });
    ...
    it('should filter visible trailers in JSON output', () => {
      ...
      expect(parsed.atoms[0].trailers.constraint).toEqual(['Must use OAuth2']);
      expect(parsed.atoms[0].trailers.confidence).toBeUndefined();
      expect(parsed.atoms[0].trailers.rejected).toBeUndefined();
    });
    ...
    it('should convert trailer keys to snake_case', () => {
      ...
      expect(parsed.atoms[0].trailers.scope_risk).toBe('wide');
      expect(parsed.atoms[0].trailers.not_tested).toEqual(['edge cases']);
      expect(parsed.atoms[0].trailers.depends_on).toEqual(['aabbccdd']);
    });
  });
  ...
  describe('formatStalenessResult', () => {
    it('should produce valid JSON with stale atoms', () => {
      ...
      const data: FormattableStalenessResult = {
        atoms: [{ atom, reasons: ['Too old', 'Low confidence'] }],
      };
      ...
      expect(parsed.stale_atoms[0].reasons).toEqual(['Too old', 'Low confidence']);
    });
  ...
  describe('formatError', () => {
    it('should produce valid JSON with error details', () => {
      ...
      expect(parsed.errors[0].field).toBeNull();
      expect(parsed.errors[1].field).toBe('intent');
    });
  });
