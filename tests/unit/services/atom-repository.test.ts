...
import type { QueryTarget, PathQueryOptions } from '../../../src/types/query.js';
...
function makeQueryTarget(overrides: Partial<QueryTarget> = {}): QueryTarget {
  return {
    raw: 'src/auth.ts',
    type: 'file',
...
describe('AtomRepository', () => {
  ...
  describe('findByTarget', () => {
    it('should return atoms for a file target', async () => {
      ...
      const result = await repo.findByTarget(target, options);
    ...
    it('should filter out non-Lore commits', async () => {
      ...
      const result = await repo.findByTarget(makeQueryTarget(), makeQueryOptions());
    ...
    it('should pass author filter to git log args', async () => {
      ...
      await repo.findByTarget(makeQueryTarget(), options);
    ...
    it('should apply author filter at the application level', async () => {
      ...
      const result = await repo.findByTarget(makeQueryTarget(), options);
  ...
  describe('git log format', () => {
    it('should use the correct NUL-separated format', async () => {
      vi.mocked(gitClient.log).mockResolvedValue([]);
    ...
    it('should include -- separator before path args in findByTarget', async () => {
      vi.mocked(gitClient.log).mockResolvedValue([]);

      const target = makeQueryTarget({ filePath: 'src/auth.ts' });
      await repo.findByTarget(target, makeQueryOptions());
