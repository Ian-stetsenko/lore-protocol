  ...
  describe('analyze', () => {
    describe('age signal', () => {
      it('should detect atoms older than the configured threshold', async () => {
        ...
        expect(result[0].reasons.some((r) => r.startsWith('Age:'))).toBe(true);
      });
      ...
      it('should respect custom age threshold', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.includes('threshold: 30d'))).toBe(true);
      });
    ...
    describe('drift signal', () => {
      it('should detect files with too many commits since the atom', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.startsWith('Drift:'))).toBe(true);
        expect(result[0].reasons.some((r) => r.includes('25 commits'))).toBe(true);
      });
      ...
      it('should check drift for each file in filesChanged', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.includes('src/db.ts'))).toBe(true);
      });
      ...
      it('should respect custom drift threshold', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.includes('threshold: 5'))).toBe(true);
      });
    });
    ...
    describe('low confidence signal', () => {
      it('should detect atoms with Confidence: low', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.includes('Low confidence'))).toBe(true);
      });
    ...
    describe('expired hints signal', () => {
      it('should detect expired [until:YYYY-MM] directives', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.includes('Expired hint'))).toBe(true);
        expect(result[0].reasons.some((r) => r.includes('until:2024-06'))).toBe(true);
      });
      ...
      it('should detect expired [until:YYYY-MM-DD] directives', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.includes('until:2024-01-15'))).toBe(true);
      });
      ...
      it('should detect multiple expired hints in different directives', async () => {
        ...
        const expiredReasons = result[0].reasons.filter((r) => r.includes('Expired hint'));
    ...
    describe('orphaned dependency signal', () => {
      it('should detect dependencies on superseded atoms', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.includes('Orphaned dependency'))).toBe(true);
        expect(result[0].reasons.some((r) => r.includes('bbbb2222'))).toBe(true);
      });
      ...
      it('should detect multiple orphaned dependencies', async () => {
        ...
        expect(result).toHaveLength(1);
        const orphanReasons = result[0].reasons.filter((r) => r.includes('Orphaned dependency'));
