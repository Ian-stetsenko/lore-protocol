...
describe('Validator', () => {
  ...
  beforeEach(() => {
    ...
    validator = new Validator(mockParser as any, config);
  });

  describe('basic validation', () => {
    it('should return valid for a correct commit', () => {
      const commit = makeCommit();
      const results = validator.validate([commit]);
    ...
    it('should validate multiple commits', () => {
      const commit1 = makeCommit({ hash: 'aaa111' });
      const commit2 = makeCommit({ hash: 'bbb222' });
      const results = validator.validate([commit1, commit2]);
  ...
  describe('Rule 1: trailer format', () => {
    it('should error when trailers cannot be parsed', () => {
  ...
  describe('Rule 2: Lore-id present', () => {
    it('should error when Lore-id is missing', () => {
  ...
  describe('Rule 3: Lore-id format', () => {
    it('should error when Lore-id is not 8-char hex', () => {
    ...
    it('should pass for valid 8-char hex Lore-id', () => {
    ...
    it('should error for too-short Lore-id', () => {
    ...
    it('should error for uppercase hex Lore-id', () => {
      mockParser.parse.mockReturnValue(makeTrailers({ 'Lore-id': 'ABCD1234' }));
  ...
  describe('Rule 4: valid enum values', () => {
    ...
    it('should error on invalid Scope-risk', () => {
      mockParser.parse.mockReturnValue(
    ...
    it('should error on invalid Reversibility', () => {
      mockParser.parse.mockReturnValue(
    ...
    it('should not error when enum trailers are null', () => {
      mockParser.parse.mockReturnValue(
        makeTrailers({
          Confidence: null,
          'Scope-risk': null,
  ...
  describe('Rule 5: intent length', () => {
    ...
    it('should not warn when intent is within limit', () => {
    ...
    it('should use config value for max length', () => {
      const customConfig: LoreConfig = {
        ...DEFAULT_CONFIG,
        validation: { ...DEFAULT_CONFIG.validation, intentMaxLength: 50 },
      };
      const customValidator = new Validator(mockParser as any, customConfig);

      const commit = makeCommit({ subject: 'a'.repeat(51) });
      const results = customValidator.validate([commit]);
  ...
  describe('Rule 6: required trailers', () => {
    it('should warn on missing required trailers (non-strict)', () => {
      ...
      const requiredValidator = new Validator(mockParser as any, requiredConfig);
      mockParser.parse.mockReturnValue(makeTrailers({ Confidence: null }));

      const commit = makeCommit();
      const results = requiredValidator.validate([commit]);
    ...
    it('should error on missing required trailers (strict)', () => {
      const strictConfig: LoreConfig = {
        ...DEFAULT_CONFIG,
        trailers: { required: ['Confidence'], custom: [] },
        validation: { ...DEFAULT_CONFIG.validation, strict: true },
      };
      const strictValidator = new Validator(mockParser as any, strictConfig);
      mockParser.parse.mockReturnValue(makeTrailers({ Confidence: null }));

      const commit = makeCommit();
      const results = strictValidator.validate([commit]);
    ...
    it('should not warn when required trailers are present', () => {
      const requiredConfig: LoreConfig = {
        ...DEFAULT_CONFIG,
        trailers: { required: ['Confidence'], custom: [] },
      };
  ...
  describe('Rule 7: message line count', () => {
    ...
    it('should not warn when within line limit', () => {
  ...
  describe('Rule 8: reference format', () => {
    it('should warn on invalid reference format', () => {
      mockParser.parse.mockReturnValue(
        makeTrailers({
          Supersedes: ['not-hex!'],
          Related: ['toolong12'],
    ...
    it('should not warn on valid reference format', () => {
      mockParser.parse.mockReturnValue(
        makeTrailers({
          Supersedes: ['aabbccdd'],
          'Depends-on': ['11223344'],
  ...
  describe('Rule 9: trailer count', () => {
    it('should warn when more than 5 of any trailer type', () => {
      mockParser.parse.mockReturnValue(
        makeTrailers({
    ...
    it('should not warn when 5 or fewer of each type', () => {
      mockParser.parse.mockReturnValue(
        makeTrailers({
  ...
  describe('overall validity', () => {
    ...
    it('should be valid even with warnings', () => {
      const commit = makeCommit({ subject: 'a'.repeat(100) });
      const results = validator.validate([commit]);
  ...
  describe('commit hash and lore-id reporting', () => {
    ...
    it('should report lore-id when present', () => {
    ...
    it('should report null lore-id when parse fails', () => {
      mockParser.parse.mockImplementation(() => {
        throw new Error('Parse error');
      });
